...
axios
		...
		.use(function (config) {
				...
				config = {
						...
						withCredentials: true, // 是否携带coocie ， true: 是， false:否
						...
						onUploadProgress: function (progressEvent) { // 允许处理上传的进度事件
								// Do whatever you want with the native progress event
						},
						onDownloadProgress: function (progressEvent) {
								// Do whatever you want with the native progress event
						},
						validateStatus: function (status) { // 自定义有效的status,
				...
				if (typeof config.method === 'string' && config.method.toLocaleUpperCase === 'GET') {
						config = {
								...config,
								params: config.data,
								data: {}
